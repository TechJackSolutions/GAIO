...
- `docs/GAIO_Verification_Guide.md`: the tag-verification guide the framework referenced but had not yet published, containing Normalization Spec v1 (all 15 rules), the tag JSON Schema, the step-by-step hash verification procedure (including the requirement to strip the three embedded hash header lines before recomputing), reference-implementation pointers, limitations, and known spec-vs-implementation discrepancies.
